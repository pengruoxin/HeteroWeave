...
class Sequential_with_size(nn.Sequential):
    def forward(self, x, size):
        for module in self:
            # Extracted Swin ranges may contain both blocks whose forward is
            # ``forward(x, hw_shape)`` and ordinary modules such as LayerNorm
            # or Linear whose forward only accepts ``x``.  Dispatch by the
            # bound forward signature instead of unconditionally injecting
            # the spatial-size argument.
            import inspect
            parameters = list(inspect.signature(
                module.forward).parameters.values())
            positional = [
            ...
            x = module(x, size) if accepts_varargs or len(positional) >= 2 \
                else module(x)
